...
# For 1b, we need to change the %include statement
sed -E 's|%include "[a-zA-Z:\_. 0-9\-]+"|TARGET|g' AD_algorithm_comparison/1b.\ Extract\ proxy\ variables\ from\ core\ HRS\ _\ 2018.01.17.sas | sed "s|TARGET|%include '`pwd`/updated_AD_algorithm_comparison/1a_extract_self_response_variables.sas'|" > updated_AD_algorithm_comparison/1b_extract_proxy_variables.sas
